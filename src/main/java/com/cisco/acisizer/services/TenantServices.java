...
import com.cisco.acisizer.exceptions.TenantNameExistsException;
...
import com.cisco.acisizer.ui.models.SharedResourceUi;
...
public class TenantServices {
	...
	private Tenant getTargetTenant(int tenantId,
			LogicalRequirement logicalRequirement) throws AciEntityNotFound {
/*		Optional<Tenant> tenantsToTemove = logicalRequirement.getTenants().stream()
				.filter(tenantToRemove -> tenantId == tenantToRemove.getId()).findFirst();
		if (!tenantsToTemove.isPresent()) {
			throw new AciEntityNotFound(ACISizerConstant.TENANT_NOT_FOUND_WITH_ID);
		}
		final Tenant tenantToDelete = tenantsToTemove.get();
		if (!tenantToDelete.getContracts().isEmpty()) {
			tenantToDelete.getContracts().stream().forEach((contract) -> {
				contractServices.updateProviderConsumers(tenantToDelete, Utility.getCommonTenant(logicalRequirement),
						contract);
			});
		}
		return tenantsToTemove;
*/	
		...
		if(null != tenantToTemove.getContracts())
		{
			...
			{
				contractServices.updateProviderConsumers(tenantToTemove, Utility.getCommonTenant(logicalRequirement),iter);
			}
		}
	...
	private boolean isDuplicate(Tenant tenant, List<Tenant> tenants) {
		/*return  tenants.stream()
				.filter(tenantIter -> (true == tenantIter.getDisplayName().equals(tenant.getDisplayName())))
				.findFirst().isPresent();*/
...
/*			commonTenant.getVrfs().parallelStream()
					.forEach(vrf -> addCommonTenantVrfToNodes(nodes, commonTenant, currTenant, vrf));
*/		
...
/*		currTenant.getVrfs().parallelStream().forEach(vrf -> addCurrentTenantVrfToNodes(nodes, currTenant, vrf));
*/
...
/*		commonTenant.getBds().parallelStream()
				.filter(bd -> bd.getVrf().equals(vrf.getName())
...
/*		currTenant.getBds().parallelStream().filter(bd -> bd.getVrf().equals(vrf.getName()))
				.forEach(bd -> addBdToNodes(nodes, currTenant, vrf, bd));
*/		
...
/*		currTenant.getL3outs().parallelStream().filter(l3out -> l3out.getVrf().equals(vrf.getName()))
				.forEach(l3out -> addL3outToNodes(nodes, currTenant, vrf, l3out));
		currTenant.getSharedResources().parallelStream()
				.filter(sharedResources -> sharedResources.getVrf().equals(vrf.getName()))
				.forEach(sharedResources -> addSharedResourceToNodes(nodes, sharedResources, currTenant));
*/
		...
		return nodes.getNodes()
				.add(vrfServices.outputmapping(vrf, new com.cisco.acisizer.ui.models.VrfUi(), currTenant));
	}
	...
	public boolean addL3outToNodes(ViewNodes nodes, Tenant currTenant, Vrf vrf, L3out l3out) {
		com.cisco.acisizer.ui.models.L3outUi l3outToAddInConnection = l3OutServices.outputMapping(l3out,
				new com.cisco.acisizer.ui.models.L3outUi(), vrf, currTenant);
	...
	public boolean addBdToNodes(ViewNodes nodes, Tenant currTenant, Vrf vrf, Bd bd) {
		com.cisco.acisizer.ui.models.BdUi bdToAddInConnection = bdServices.outputMapping(bd, currTenant, vrf);
	...
	public AciSizerModelUi getDefaultValues(Tenant tenant, Tenant commonTenant,String type) throws AciEntityNotFound {
		...
		case ACISizerConstant._app:
			...
			/*List<String> tenantNames = tenant.getApps().parallelStream().map(app -> app.getDisplayName())
		...
		case ACISizerConstant._bd:
			...
			/*List<String> bdNames = tenant.getBds().parallelStream().map(bd -> bd.getDisplayName())
					.collect(Collectors.toList());*/
			names=new ArrayList<>();
		...
		case ACISizerConstant._contract:
			...
			/*List<String> contractNames = tenant.getContracts().parallelStream()
					.map(contract -> contract.getDisplayName()).collect(Collectors.toList());*/
			names=new ArrayList<>();
			for (Contract contract : tenant.getContracts()) {
		...
		case ACISizerConstant._epg:
			...
			/*List<String> epgNames = tenant.getEpgs().parallelStream().map(epg -> epg.getDisplayName())
					.collect(Collectors.toList());*/
			names=new ArrayList<>();
			for (Epg epg : tenant.getEpgs()) {
		...
		case ACISizerConstant._l3out:
			...
			/*List<String> l3outNames = tenant.getL3outs().parallelStream().map(l3out -> l3out.getDisplayName())
					.collect(Collectors.toList());*/
			names=new ArrayList<>();
		...
		case ACISizerConstant._vrf:
			...
			/*List<String> vrfNames = tenant.getVrfs().parallelStream().map(vrf -> vrf.getDisplayName())
					.collect(Collectors.toList());*/
			names=new ArrayList<>();
			for (Vrf vrf : tenant.getVrfs()) {
	...
	private void setDefaultL3outConsumer(Tenant commonTenant,
			ContractUi contractL3outUi) {
		...
		for(L3out l3out : commonTenant.getL3outs())
		{
			if(l3out.getId() == ACISizerConstant.DEFAULT_L3OUT_ID)
			{
				...
				contractL3outUi.setConsumerName(commonTenant.getDisplayName()+ACISizerConstant.TENANT_FIELD_SEPERATOR + l3out.getDisplayName());
	...
	private void setDefaultSharedResource(EpgUi epgUi, Tenant commonTenant) {
		...
		for(SharedResource sr : commonTenant.getSharedResources())
		{
			if(sr.getId() == ACISizerConstant.DEFAULT_SHARED_RESOURCE_ID)
			{
				...
				epgUi.setSharedServiceName(commonTenant.getDisplayName() + ACISizerConstant.TENANT_FIELD_SEPERATOR+  sr.getDisplayName());
