      ...
      fn.showContractSelection = function(scope){
        ...
        var modalInstance = $uibModal.open({
          ...
          controller: ['$scope', 'util', 'repoFilterOptions', 'paginationOptions', 'repoTestService', 'OBJECT_TYPES', function($scope, util, repoFilterOptions, paginationOptions, repoTestService, OBJECT_TYPES) {
            ...
            vm.selectRow = function(rowIndex, repoObject){
              vm.selectedRowIndex = rowIndex; 
              vm.selectedContract = repoObject;
               vm.selectedRow = rowIndex;
              // fn.showObjectDetails($scope.$new(), repoObject, 'contract');
              ...
              repoTestService.getObjectDetails(repoObject.id, objType).then(function(detailsObject){
                fn.showObjectDetails($scope.$new(), detailsObject, repoObject.objectType);
              });
            };
        ...
        var modalInstance = $uibModal.open({
          ...
          controller : ['$scope', 'projectDetailsService','$rootScope', function($scope, projectDetailsService,$rootScope){
            ...
            vm.pushToApic = function(){
              console.log(vm.selectedApicDevice.name);
              if(vm.selectedTenant && vm.selectedApicDevice){
                projectDetailsService.pushToApic(vm.selectedTenant.id, vm.selectedApicDevice.id).then(function(){
                  $rootScope.apicDevice=vm.selectedApicDevice;
